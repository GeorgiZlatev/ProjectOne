...
int check_game_over(char** matrix, int round, char empty_symbol) {
	...
	int wiinner[winner_conditions];
	wiinner[0] = chaeck_rows(matrix, empty_symbol);
	wiinner[1] = chaeck_columns(matrix, empty_symbol);
	wiinner[2] = chaeck_main_diagonal(matrix, empty_symbol);
	wiinner[3] = check_secondary_diagonal(matrix, empty_symbol);

	for (int i = 0; i < winner_conditions; i++)
	{
		if (wiinner[i] != 0) {
			print_char_matrix(matrix, 1);
			if (wiinner[i] == PlayerX) {
				printf("Winner id: X");
				return PlayerX;
			}
			else
